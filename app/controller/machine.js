...
module.exports = app => ({
	async store(req, res) {
		...
		console.log("FILES => ", machinesFiles, productRefFiles, sewingTypeFiles)
		const {
			name,
			manufacturer,
			description,
		...
		if (machines.length > 0) {
			const responseMachines = machines.map(machine => {
				return {
					...
					productRef: machine.productRef ? machine.productRef.map(img => `${process.env.STATIC_FILES_URL}machines/product_ref${img}`) : null,
					sewingType: machine.sewingType ? `${process.env.STATIC_FILES_URL}machines/sewing_type${machine.sewingType}` : null,
