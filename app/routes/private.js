...
module.exports = (gl) => {
  ...
  gl.route("/reseller").post(reseller.store);
  ...
  gl.route("/news/:id").put(news.update).delete(news.del);
  ...
  gl.route("/tools").post(
    multer({ store: memoryStorage }).array("toolsFiles", 6),
    tools.store
  );
  gl.route("/tools/:id")
    .delete(tools.delete)
    .put(multer({ store: memoryStorage }).array("toolsFiles", 6), tools.update);

  gl.route("/settings/").post(settings.store);
  gl.route("/settings/:id").put(settings.update);
};
